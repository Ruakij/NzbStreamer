...
| `WEBDAV_IDLE_TIMEOUT`               | 5s                    | How long a finished reader is kept for the next request of the same file, so a client reading in Range spans reuses a warm readahead window instead of building one per span; 0 closes it with the request that opened it                                                                                                                                                |
| `WEBDAV_MAX_IDLE_READERS`           | 4                     | Kept readers across every file, since each one holds up to `READAHEAD_MAX_SIZE`; the oldest goes when a new one does not fit                                                                                                                                                                                                                                             |
...
| `SABNZBD_DELETE_ON_REMOVE`          | false                 | Removing a download from a client's history deletes the nzb and takes its files off the mount, rather than archiving it.                                                                                                                                                                                                                                                 |
| `SABNZBD_ADD_WAIT`                  | 30s                   | How long an add is held open to fail before it is answered as accepted, so a client that grabbed a broken release is told while it is still choosing one instead of a queue refresh later; one that takes longer is answered with its id and polled for as usual. Keep it under the client's own request timeout; 0 answers every add immediately                        |
| **Cache**                           |
...
| `CACHE_WRITEBACK_SIZE`              | 0                     | Bytes the write-back buffer may hold in memory before being drained to disk; 0 writes each segment through to disk synchronously instead                                                                                                                                                                                                                                                                                                  |
| **Library**                         |
| `LIBRARY_MAX_BYTES`                 | 0                     | Nominal size the library may reach before further nzbs are refused; 0 is unlimited <br>The nominal size is what every nzb added describes, not what is on disk, so this bounds how far the library is overprovisioned against `CACHE_MAX_SIZE`. A full library still serves everything it holds and reports degraded                                                     |
| **Metrics**                         |
| `METRICS_ACTIVE_WINDOWS`            | 24h,168h,720h         | Spans the working-set metrics are bucketed over, one `window` series each: {`library_active_bytes`, `library_active_segments`}                                                                                                                                                                                                                                           |
| `METRICS_OTLP_ENDPOINT`             |                       | Collector the metrics are pushed to, as `scheme://host:port`                                                                                                                                                                                                                                                                                                             |
| `METRICS_OTLP_PROTOCOL`             | grpc                  | `grpc`, or `http` for OTLP over HTTP with protobuf                                                                                                                                                                                                                                                                                                                       |
| `METRICS_OTLP_INTERVAL`             | 60s                   | How often metrics are pushed; every push is a full collection                                                                                                                                                                                                                                                                                                            |
| `METRICS_SERVICE_NAME`              | nzbstreamer           | Identification name to a collector                                                                                                                                                                                                                                                                                                                                       |
| **Metadata**                        |
...
| **Health-Probing**                  |
| `PROBE_ADD_CONFIDENCE`             | 0.25                  | Segments probed per content group at add time, as a fraction of its segments, spread evenly, so only a missing run a few segments long can hide <br>0 disables checking                                                                                                                                                                                                                 |
| `PROBE_ADD_PARALLELISM`            | 0                     | Concurrent segment-checks during an add; defaults to the sum of the servers connections when 0 or less                                                                                                                                                                                                                                                                                                                    |
| `PROBE_PERIODIC_CONFIDENCE`        | 0.99                  | Segments probed per content group by the background pass, as a fraction of its segments, continuing what the add covered <br>With the tolerance at zero it is also the covered fraction, so 0.99 is very nearly a census, and STAT rides a pipelined connection, so a census is seconds rather than a download. Raising it picks up where the add stopped; lowering it leaves groups above target and skipped. 0 scans nothing |
| `PROBE_PERIODIC_PARALLELISM`       | 0                     | Concurrent segment-checks of the background pass, which competes with reads for the connections; defaults to the sum of the servers connections when 0 or less <br>This is the first thing to turn down on a metered or busy server                                                                                                                                                                                       |
| `PROBE_PERIODIC_INTERVAL`          | 168h                  | How often the background pass runs, and how long an answer counts as fresh <br>0 or less disables the pass. A group already failed is never re-probed, except one whose posts were younger than `PROBE_MIN_AGE` when they missed, which is re-asked once a retry has come round                                                                                                                                           |
| `PROBE_MIN_AGE`                    | 24h                   | How old a post must be for a missing article to count as gone rather than as not yet arrived <br>A miss on a younger post still fails the group and drops its files, but records a retry instead of a verdict, so the background pass re-asks once the post is old enough                                                                                                                                                  |
| `PROBE_MAX_MISSING_PERCENT`        | 100                   | Reserved for a future repair tolerance, and unused while the tolerance is zero                                                                                                                                                                                                                                                                                                                                            |
| `PROBE_PAR2_SAFETY`                | 0.9                   | Reserved for a future repair tolerance, and unused while the tolerance is zero                                                                                                                                                                                                                                                                                                                                            |
| `PROBE_UNDECIDED_ACCEPT`           | true                  | Reserved for a future repair tolerance, and unused while the tolerance is zero                                                                                                                                                                                                                                                                                                                                            |
| **Filesystem-Options**              |
| `FILESYSTEM_BLACKLIST`              | (?i)\.par2$           | Late Regex-blacklist, applied on the actual file added to the filesystem; includes files from archives <br>Can be used to hide archive-files, but leaving unpacked files. Hides .par2 by default; they are not probed, since a missing recovery file changes nothing while the tolerance is zero                                                                                                                                  |
